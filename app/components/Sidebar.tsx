...
export default function Sidebar() {
  ...
  return (
    <>
      <aside className="hidden md:flex w- bg-white border-r border-gray-100 h-screen flex-col fixed left-0 top-0">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
          {menu.map((item) => {
            const isActive = pathname === item.href || (item.name === "Inbox" && pathname === "/");
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text- font-medium transition-all ${
                  isActive? "bg-green-50 text-green-700" : "text-gray-500 hover:bg-gray-50 hover:text-gray-700"
                }`}
              >
                <span className="text- w-6 text-center">{item.icon}</span>
                {item.name}
              </Link>
            );
          })}
